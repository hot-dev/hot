...
Web/custom POST ─┤                              │                                  ├── callback or stream event
                 │                              │                                  │
…future buses ───┘                              └── ::transport/show-typing        └── …
```
...
|---|---|---|
| **Adapter** | `telegram_adapter.hot`, `slack_adapter.hot`, `web_adapter.hot` | Its transport's API, message shapes, delivery/callback/stream behavior, deep-link format |
...
The adapter files are treated as **library code**: they expose
pure callable functions (`record-voice`, `check-updates`,
`on-telegram-update`, `on-slack-event`, `on-web-message`, …) but do **not** carry
...
```json
{
  "session_id": "acme-general",
  "text": "/ask what did we decide about launch?",
  "user_id": "u_123",
  "user_name": "Ada",
  "message_id": "m_456",
  "reply_mode": "stream"
...
**Event Subscriptions** settings after deploying.
...
├── web_adapter.hot        # Custom web/chat apps: normalized webhook input,
│                            callback or stream-event replies
...
hot/pkg/hot-ai/             # Reusable AI primitives (session, memory, rag, bus, skill, …)
hot/pkg/hot-ai-agent/       # Reusable agent harness (transport, command, runtime, stream, …)
