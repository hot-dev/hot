...
pub async fn webhook_services_list_handler(
    ...
    breadcrumbs.push(templates::BreadcrumbItem::current("Webhooks".to_string()));
...
pub async fn webhook_service_detail_handler(
    ...
    let base_url = format!("{}/webhook/{}/{}/{}", api_url, org_slug, env_name, service);
    ...
    let (endpoints, total_endpoints) = if let Some(env_id) = env_id {
        ...
        let all_endpoints = Webhook::get_by_env(&db, &env_id).await.unwrap_or_else(|e| {
            tracing::error!("Failed to get webhook endpoints for env {}: {}", env_id, e);
            Vec::new()
        });
    ...
    let pagination = list_query::PaginationWindow::new(total_endpoints, &page);
